# [←](../README.md) <a id="Home"></a> JetBrains MPS Introduction
...
Итак, собственно, начать можно с того, почему оно называется MPS. А называется оно так от Meta Programming System - то есть система метапрограммирования. "Мета" - означает, что мы пишем данные, которые описывают данные. То есть при помощи JetBrains MPS мы можем описать то, как должен выглядеть язык.
...
Всё, что находится в модуле описывает модель, но с разных сторон. Например, editor описывает, как показать модель в редакторе. Но наш путь начинается с секции **"structure"**. Данная секция содержит структуру нашего языка, то есть описывает синтаксис нашего языка.
В MPS структура языка описывается при помощи "кирпичиков", которые называются "концептами" (**Concept**). Как мы помним, в MPS мы работаем с AST, которое состоит из node (англ. "узел"). Так вот, каждая node, как раз-таки, и выражена определённым концептом.
...
Начнём с создания нашего главного, центрального, корневого концепта - Калькулятор.
Через контекстное меню модели "structure" выберем **"New → Concept"**. Получим:

![](./img/01_NonameConcept.png)

Мы не можем писать текст там, где захотим. Мы можем редактировать только элементы. Например, нам предлагают заменить ``<no name>`` на название концепта. Это и есть Projection Editor. То есть у нас есть концепт, который представляет часть модели нашего языка. И мы редактируем этот концепт через его представление или проекцию.

Вместо **"no name"** укажем название "Calculator". Как и классы в Java, будем называть концепты с большой буквы. Стоит так же привыкнуть к тому, что JetBrains MPS подобно IntelliJ Idea IDE умеет подсказывать варианты, которые можно использовать в том или ином месте.
Получим подсказку, встав курсором в место, где нужна подсказка, и нажав **Ctrl + Space**:

![](./img/02_ConceptInterfaceHint.png)

Выберем для концепта калькулятора интерфес **INamedConcept**, что будет означать, что наш концепт будет именованным. Если мы с Ctrl кликнем по названию интерфейса, то мы откроем его представление (как в IntelliJ Idea открыли бы код). Мы видим, что INamedConcept добавляет параметр (property) с названием ``name``, то есть и калькулятор (т.к. реализует INamedConcept) будет иметь property с названием name, т.е. имя.
...
Концепт поля Input именованный, т.к. мы захотим как-то обращаться к нему из других полей (т.е. из Output). Самим Output-полям имена не нужны, т.к. к ним никто не будет обращаться.
...
Хорошо, у нас появилась структура нашего языка, описанная концептами. Но как их смогут использовать? Какие property можно редактировать? Чтобы предоставить пользователю нашего языка удобный механизм редактирования node, которые будут выражены определёнными концептами в MPS, используется **Concept Editor**.
...
Вместо ``choose cell model`` напишем **const** и воспользуемся подсказкой MPS, нажав **Ctrl + Space**:
...
Итак, встанем курсором на ячейку с константным значением и нажмём **Enter** для добавления новой ячейки.
...
Выберем нашу ``property`` name, которая в нашем концепте InputField есть благодаря тому, что он наследуется от INamedConcept.

Аналогичные действия выполним для OutputField, только без действий с property. Как мы помним, наш OutputField - неименованный концепт, т.к. нам не нужно ссылаться на OutputField, только из него нужно будет ссылаться.
Таким образом, редактор концепта OutputField будет выглядеть так:

![](./img/09_OutputFieldEditor.png)

Теперь опишем редактор калькулятора. Ведь именно в нём будут содержаться поля. Следовательно, в редакторе концепта Calculator нужно описать то, как именно пользователь сможет описывать калькулятор.
...
Кроме того, так как input полей будет несколько, мы хотим, чтобы они добавлялись друг под другом, то есть каждый children на новой строке. Поэтому встаём на ячейке с указанием InputField, нажимаем **Alt + Enter** для вызова **Intention Menu** и выбираем **Add New Line for Children**:
...
Для этого встаём на символ ``-)`` завершения первой коллекции inputField, нажимаем Enter. Это добавит ячейку, но, правда, она будет на одной строке с outputField. Это мы сейчас исправим. Зададим пустое значение constant для ячейки (так же, как делали для inputField, но оставляем без текста), после чего вызываем Intention меню (через **Alt + Enter**) и выбираем **Add new line**:

![](./img/13_SeparateCalculatorChildrens.png)

После всех "махинаций" у нас должен получиться следующий результат:

![](./img/14_CalculatorEditorResult.png)

Ура, вот мы с вами немного подружились с концептами. Давайте опробуем, как же мы будем работать с нашим языком. Вспоминаем, что у нас был не только Language модуль, но ещё и Solution sandbox модуль (песочница):

![](./img/15_SolutionSandbox.png)

Кстати, мы у модулей видим приписку - **generation required**. Это очень немаловажное замечание от JetBrains MPS. Это означает, что в нашем языке были сделаны изменения, и нужно модуль пересобрать. Чтобы пересобрать модуль, выбираем его в дереве проекта Calculator, вызываем при помощи правой кнопки мыши контекстное меню и выбираем **Rebuild**.
...
После этого на вкладке **"Dependencies"** добавляем **"jetbrains.mps.baseLanguage"** и указываем, что **Scope - Extends**.
...
Опишем новый концепт "Ссылка на поле ввода", назвав его **InputFieldReference**:
...
После этого в Sandbox Solution мы cможем в output использовать выражения:
...
Наш DSL не является самоцелью. Нам нужен DSL, чтобы описать какой-либо результат. JetBrains MPS использует для этого генератор (**generator**). То есть, на основе описания DSL генерируется некоторый результат, и мы должны описать, каким образом.
...
По умолчанию, при создании проекта генератор уже есть:

![](./img/22_DefaultGenerator.png)

Нас интересует внутри элемента **"main@generator"** элемент **main**.
Это - точка входа (**entry point**, как метод main() в Java) в генератор. Здесь указано, какие узлы нужны трансформировать, и как.
...
Выбираем для этого в дереве проекта модуль нашего языка (с иконкой **L**) и в контекстном меню выбираем **Module Properties**. Можно также выбрать модуль и просто нажать **Alt+Enter**.

На вкладке **dependencies** добавляем ещё одну зависимость:
``jetbrains.mps.execution.util``
Scope ей нужно выставить в **Extends**:

![](./img/26_LanguageDependencies.png)

Теперь дело за малым. Т.к. наш класс CalculatorImpl получает в качестве input концепт калькулятора (Calculator), то именно он должен реализовывать интерфейс IMainClass, который и позволит нам запускать код прямо из MPS.
...
Чтобы импортировать их, откроем контекстное меню для **main@generator** и выберем **"Model Properties"**. Зависимости - это не язык. Поэтому, воспользуемся вкладкой **"Dependencies"**. Добавим там вышеуказанные зависимости.
...
```
И теперь изменим наш метод main:
```java
public static void main(string[] args) {
...
Укажем вместо ``no statements`` значение **node.inputField**. Как мы помним, node для нас - концепт Calculator (указан у нас в input). А inputField указан в блоке children (т.е. там хранится коллекция всех inputField'ов):
...
Далее, мы снова выполняем LOOP по node типа InputField. В этом цикле мы, зная node, хотим получить ссылку на те самые поля, которые мы создали ранее. Вот именно для этого FieldDeclaration'ы нам и нужны.
...
```
После этого выделяем строчку целиком (вместе с точкой с запятой на конце), выбираем **Add Node Macro**:
...
Как мы понимаем, просто цикл сгенерирует 4 одинаковые по названию переменные, что приведёт к падению компиляции. Поэтому, зададим уникальные имена, как мы это делали раньше. Выберем переменную i, в Intention меню (по Atl + Enter) выберем **Property Macro**. Как мы помним, в инспекторе надо взять из контекста генерации genContext уникальные названия по некоторому основанию. Сделаем это:

![](./img/48_UniqueLocalVariableName.png)

Теперь нужно эти переменные как-то инициализировать. Для этого на них нужно будет как-то сослаться. А это значит, что нам снова понадобятся Label'ы:
...
Теперь ниже нам понадобится код, который будет инициализировать эти локальные переменные значениями, полученными из input-полей.
...
Перехватывать будем **NumberFormatException**, а в try блоке добавим:
```java
i = Integer.parseInt(inputField.getText());
```
Выделяем эту строчку полностью (от **i** до точки запятой включительно) и через Intention меню (**Alt + Enter**) добавляем Node Macro **$LOOP$** с циклом по **node.inputField**.

Далее, как мы делали ранее, делаем Reference Macro для **i**, чтобы сослаться на переменную **i**, созданную ранее в цикле, и так же для inputField, чтобы сослаться на соответствующее этой **i** поле ввода:

![](./img/52_LocalVariableReference.png)

Добавим теперь под блоком try-catch строку:
```java
...
Теперь, заменим аргумент ``null`` в методе setText на ``"" + (null)``.
...
```
Заполним содержимое блока следующим простым кодом:
```java
int i;
i = 1 + i;
```
...
Теперь давайте проверим, всё ли правильно мы написали.
...
Если мы создадим другой калькулятор (т.е. новый root node калькулятора) в sandbox solution нашем, мы сможем обращаться к input-полям другого калькулятора, что неправильно.
...
```
После того, как мы указали, что Scope у InputFieldReference для поиска InputField наследуется, мы должны указать, что наш Calculator - это **ScopeProvider**, т.е. провайдер этого Scope. Для этого концепт Calculator должен имплементировать также и интерфейс ScopeProvider.
Таким образом наш Калькулятор будет предоставлять свой Scope всем своим вложенным InputFieldReferences (как мы помним, наша модель представляет собой AST).
...
Нужно это потому, что ранее для поля Output мы разрешили указывать Expression.
...
Теперь, после того как мы сделаем **Rebuild** для нашего Language модуля, MPS нам больше не даст указывать неправильный тип так, как мы делали это раньше:
...
На Youtube есть видео про написание калькулятора: "[The introductory Calculator Language Tutorial](https://www.youtube.com/watch?v=T1RErEvbgRc)".
